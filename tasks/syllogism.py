""" ### syllogism
Generates syllogistic reasoning tasks

Classical logic puzzles with categorical statements (All, Some, No).
...
````
Example 1:
Question: Given the premises: "All A are B" and "All B are C", what can you conclude?
Answer: All A are C
Metadata: {'source_dataset': 'syllogism', 'source_index': 0, 'num_premises': 2, 'difficulty': {'num_premises': 2}}

Example 2:
Question: Given the premises: "Some A are B" and "All B are C", what can you conclude?
Answer: Some A are C
Metadata: {'source_dataset': 'syllogism', 'source_index': 1, 'num_premises': 2, 'difficulty': {'num_premises': 2}}

Example 3:
Question: Given the premises: "No A are B" and "All C are A", what can you conclude?
Answer: No C are B
Metadata: {'source_dataset': 'syllogism', 'source_index': 2, 'num_premises': 2, 'difficulty': {'num_premises': 2}}
````
"""
import json
import os
...
def init_data():
    ...
    test_data = []
    # Use default parameters with different invalid_ratio for difficulty
    data = reasoning_gym.create_dataset('syllogism', size=70, seed=42, invalid_ratio=0.3)
    ...
    data = reasoning_gym.create_dataset('syllogism', size=70, seed=42, invalid_ratio=0.4, inversion_probability=0.3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('syllogism', size=70, seed=42, invalid_ratio=0.5, inversion_probability=0.5)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/syllogism")  # uncomment to upload to HuggingFace Hub

    val_data = []
    data = reasoning_gym.create_dataset('syllogism', size=120, seed=42, invalid_ratio=0.3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/syllogism")  # uncomment to upload to HuggingFace Hub

    train_data = []
    data = reasoning_gym.create_dataset('syllogism', size=500, seed=42, invalid_ratio=0.3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('syllogism', size=500, seed=42, invalid_ratio=0.4, inversion_probability=0.3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/syllogism")  # uncomment to upload to HuggingFace Hub

def load_ds():
    train = load_dataset("anonymous/syllogism", split="validate")
    test = load_dataset("anonymous/syllogism", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    # init_data()
    parser = argparse.ArgumentParser(description='Syllogism evaluation with tool generation')
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/syllogism/test', help='Experiment directory')
