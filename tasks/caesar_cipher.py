...
def init_data():
    ...
    test_data = []
    data = reasoning_gym.create_dataset('caesar_cipher', size=70, seed=42, min_words=3, max_words=10, min_rotation=1, max_rotation=10)
    ...
    data = reasoning_gym.create_dataset('caesar_cipher', size=70, seed=42, min_words=8, max_words=15, min_rotation=8, max_rotation=18)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('caesar_cipher', size=70, seed=42, min_words=12, max_words=20, min_rotation=15, max_rotation=25)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/caesar_cipher")  # uncomment to upload to HuggingFace Hub

    val_data = []
    data = reasoning_gym.create_dataset('caesar_cipher', size=120, seed=42, min_words=3, max_words=12, min_rotation=1, max_rotation=15)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/caesar_cipher")  # uncomment to upload to HuggingFace Hub

    train_data = []
    data = reasoning_gym.create_dataset('caesar_cipher', size=500, seed=42, min_words=3, max_words=10, min_rotation=1, max_rotation=10)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('caesar_cipher', size=500, seed=42, min_words=8, max_words=15, min_rotation=8, max_rotation=18)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/caesar_cipher")  # uncomment to upload to HuggingFace Hub

def load_ds():
    train = load_dataset("anonymous/caesar_cipher", split="validate")
    test = load_dataset("anonymous/caesar_cipher", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    # init_data()
    parser = argparse.ArgumentParser(description='Caesar Cipher evaluation with tool generation')
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/caesar_cipher/test', help='Experiment directory')
