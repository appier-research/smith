...
def init_data():
    ...
    test_data = []
    data = reasoning_gym.create_dataset('bitwise_arithmetic', size=70, seed=42, difficulty=2)
    ...
    data = reasoning_gym.create_dataset('bitwise_arithmetic', size=70, seed=42, difficulty=3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0    
    ...
    data = reasoning_gym.create_dataset('bitwise_arithmetic', size=70, seed=42, difficulty=4)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0    
    ...
    data = reasoning_gym.create_dataset('bitwise_arithmetic', size=70, seed=42, difficulty=5)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0    
    ...
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/bitwise_arithmetic")  # uncomment to upload to HuggingFace Hub

    val_data = []
    data = reasoning_gym.create_dataset('bitwise_arithmetic', size=120, seed=42, difficulty=2)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0    
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/bitwise_arithmetic")  # uncomment to upload to HuggingFace Hub

    train_data = []
    data = reasoning_gym.create_dataset('bitwise_arithmetic', size=500, seed=42, difficulty=2)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0    
    ...
    data = reasoning_gym.create_dataset('bitwise_arithmetic', size=500, seed=42, difficulty=3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/bitwise_arithmetic")  # uncomment to upload to HuggingFace Hub



def load_ds():
    train = load_dataset("anonymous/bitwise_arithmetic", split="validate")
    test = load_dataset("anonymous/bitwise_arithmetic", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/bitwise_arithmetic/test', help='Experiment directory')
