...
Dataset that generates tasks testing understanding of polynomial equations.

    Generates polynomial equations with:
    - Quadratic and cubic equations
    - Various coefficient ranges
    - Real solutions requiring solving techniques

    The difficulty parameter controls equation complexity:
    - Level 1: Simple quadratic equations like x^2 + 5x + 6 = 0
    - Level 2: More complex quadratics with decimal coefficients
    - Level 3+: Cubic equations and higher complexity

    Each task provides:
    - A question asking to solve the polynomial equation
    - The correct solution(s)
    - Metadata including the equation parameters

    The dataset verifies answers by checking if they satisfy the equation.
...
````
Example 1:
Question: Solve the polynomial equation: x^2 + 5x + 6 = 0. Provide all real solutions separated by commas.
Answer: -2, -3
Metadata: {'source_dataset': 'polynomial_equations', 'source_index': 0, 'equation': 'x^2 + 5x + 6 = 0', 'difficulty': {'difficulty': 2}}

Example 2:
Question: Solve the polynomial equation: x^2 - 7x + 12 = 0. Provide all real solutions separated by commas.
Answer: 3, 4
Metadata: {'source_dataset': 'polynomial_equations', 'source_index': 1, 'equation': 'x^2 - 7x + 12 = 0', 'difficulty': {'difficulty': 2}}

Example 3:
Question: Solve the polynomial equation: x^2 + 2x - 8 = 0. Provide all real solutions separated by commas.
Answer: 2, -4
Metadata: {'source_dataset': 'polynomial_equations', 'source_index': 2, 'equation': 'x^2 + 2x - 8 = 0', 'difficulty': {'difficulty': 2}}
...
def init_data():
    ...
    test_data = []
    data = reasoning_gym.create_dataset('polynomial_equations', size=70, seed=42, min_terms=2, max_terms=3, min_value=1, max_value=10, min_degree=1, max_degree=2)
    ...
    data = reasoning_gym.create_dataset('polynomial_equations', size=70, seed=42, min_terms=3, max_terms=4, min_value=1, max_value=20, min_degree=2, max_degree=3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('polynomial_equations', size=70, seed=42, min_terms=4, max_terms=5, min_value=1, max_value=30, min_degree=2, max_degree=4)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('polynomial_equations', size=70, seed=42, min_terms=5, max_terms=6, min_value=1, max_value=50, min_degree=3, max_degree=5)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/polynomial_equations")  # uncomment to upload to HuggingFace Hub

    val_data = []
    data = reasoning_gym.create_dataset('polynomial_equations', size=120, seed=42, min_terms=2, max_terms=3, min_value=1, max_value=10, min_degree=1, max_degree=2)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/polynomial_equations")  # uncomment to upload to HuggingFace Hub

    train_data = []
    data = reasoning_gym.create_dataset('polynomial_equations', size=500, seed=42, min_terms=2, max_terms=3, min_value=1, max_value=10, min_degree=1, max_degree=2)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('polynomial_equations', size=500, seed=42, min_terms=3, max_terms=4, min_value=1, max_value=20, min_degree=2, max_degree=3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/polynomial_equations")  # uncomment to upload to HuggingFace Hub



def load_ds():
    train = load_dataset("anonymous/polynomial_equations", split="validate")
    test = load_dataset("anonymous/polynomial_equations", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Polynomial Equations evaluation with tool generation')
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/polynomial_equations/test', help='Experiment directory')
