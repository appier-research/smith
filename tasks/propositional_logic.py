""" ### propositional_logic
Generates propositional logic evaluation tasks

Evaluates logical propositions using operators: AND, OR, NOT, IMPLIES, IFF (if and only if)
...
````
Example 1:
Question: Given P=True and Q=False, evaluate: (P AND Q) OR NOT P
Answer: False
Metadata: {'source_dataset': 'propositional_logic', 'source_index': 0, 'num_variables': 2, 'max_depth': 2, 'difficulty': {'num_variables': 2, 'max_depth': 2}}

Example 2:
Question: Given P=True, Q=False, and R=True, evaluate: (P IMPLIES Q) AND R
Answer: False
Metadata: {'source_dataset': 'propositional_logic', 'source_index': 1, 'num_variables': 3, 'max_depth': 2, 'difficulty': {'num_variables': 3, 'max_depth': 2}}

Example 3:
Question: Given P=False and Q=True, evaluate: P IFF (NOT Q)
Answer: False
Metadata: {'source_dataset': 'propositional_logic', 'source_index': 2, 'num_variables': 2, 'max_depth': 2, 'difficulty': {'num_variables': 2, 'max_depth': 2}}
````
"""
import json
import os
...
def init_data():
    ...
    test_data = []
    data = reasoning_gym.create_dataset('propositional_logic', size=150, seed=42, min_vars=2, max_vars=2, min_statements=2, max_statements=3, min_complexity=1, max_complexity=2)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # Skip entries with None answers
        if x['answer'] is None:
            continue
    ...
    data = reasoning_gym.create_dataset('propositional_logic', size=150, seed=42, min_vars=3, max_vars=3, min_statements=3, max_statements=4, min_complexity=2, max_complexity=3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        if x['answer'] is None:
            continue
        ...
        x['difficulty'] = 3
        test_data.append(x)
        if len(test_data) >= 140:  # 70 from first batch + 70 from this batch
            break

    data = reasoning_gym.create_dataset('propositional_logic', size=150, seed=42, min_vars=4, max_vars=4, min_statements=4, max_statements=5, min_complexity=3, max_complexity=4)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        if x['answer'] is None:
            continue
        ...
        x['difficulty'] = 4
        test_data.append(x)
        if len(test_data) >= 210:  # 70+70+70
            break
    test_data = dedup_by_question(test_data)
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/propositional_logic")  # uncomment to upload to HuggingFace Hub

    val_data = []
    data = reasoning_gym.create_dataset('propositional_logic', size=250, seed=42, min_vars=2, max_vars=2, min_statements=2, max_statements=3, min_complexity=1, max_complexity=2)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        if x['answer'] is None:
            continue
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/propositional_logic")  # uncomment to upload to HuggingFace Hub

    train_data = []
    data = reasoning_gym.create_dataset('propositional_logic', size=1000, seed=42, min_vars=2, max_vars=2, min_statements=2, max_statements=3, min_complexity=1, max_complexity=2)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        if x['answer'] is None:
            continue
    ...
    data = reasoning_gym.create_dataset('propositional_logic', size=1000, seed=42, min_vars=3, max_vars=3, min_statements=3, max_statements=4, min_complexity=2, max_complexity=3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        if x['answer'] is None:
            continue
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/propositional_logic")  # uncomment to upload to HuggingFace Hub

def load_ds():
    train = load_dataset("anonymous/propositional_logic", split="validate")
    test = load_dataset("anonymous/propositional_logic", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    # init_data()
    parser = argparse.ArgumentParser(description='Propositional Logic evaluation with tool generation')
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/propositional_logic/test', help='Experiment directory')
