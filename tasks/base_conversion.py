...
def init_data():
    ...
    test_data = []
    data = reasoning_gym.create_dataset('base_conversion', size=70, seed=42, min_base=2, max_base=10, min_value=1, max_value=500)
    ...
    data = reasoning_gym.create_dataset('base_conversion', size=70, seed=42, min_base=2, max_base=16, min_value=500, max_value=2000)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('base_conversion', size=70, seed=42, min_base=2, max_base=16, min_value=2000, max_value=5000)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/base_conversion")  # uncomment to upload to HuggingFace Hub

    val_data = []
    data = reasoning_gym.create_dataset('base_conversion', size=120, seed=42, min_base=2, max_base=12, min_value=1, max_value=1000)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/base_conversion")  # uncomment to upload to HuggingFace Hub

    train_data = []
    data = reasoning_gym.create_dataset('base_conversion', size=500, seed=42, min_base=2, max_base=10, min_value=1, max_value=500)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('base_conversion', size=500, seed=42, min_base=2, max_base=16, min_value=500, max_value=2000)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/base_conversion")  # uncomment to upload to HuggingFace Hub

def load_ds():
    train = load_dataset("anonymous/base_conversion", split="validate")
    test = load_dataset("anonymous/base_conversion", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    # init_data()
    parser = argparse.ArgumentParser(description='Base Conversion evaluation with tool generation')
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/base_conversion/test', help='Experiment directory')
