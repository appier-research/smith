""" ### ab
Generates AB pattern recognition tasks
...
````
Example 1:
Question: Continue the pattern: a b a b a b ?
Answer: a
Metadata: {'source_dataset': 'ab', 'source_index': 0, 'pattern': 'ab', 'length': 6, 'difficulty': {'length': (3, 10)}}

Example 2:
Question: Continue the pattern: a a b a a b ?
Answer: a
Metadata: {'source_dataset': 'ab', 'source_index': 1, 'pattern': 'aab', 'length': 6, 'difficulty': {'length': (3, 10)}}

Example 3:
Question: Continue the pattern: a b b a b b ?
Answer: a
Metadata: {'source_dataset': 'ab', 'source_index': 2, 'pattern': 'abb', 'length': 6, 'difficulty': {'length': (3, 10)}}
````
"""
import json
import os
...
def init_data():
    ...
    test_data = []
    # length parameter controls pattern length
    data = reasoning_gym.create_dataset('ab', size=70, seed=42, length=8)
    ...
    data = reasoning_gym.create_dataset('ab', size=70, seed=42, length=12)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('ab', size=70, seed=42, length=16)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/ab")  # uncomment to upload to HuggingFace Hub

    val_data = []
    data = reasoning_gym.create_dataset('ab', size=120, seed=42, length=10)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/ab")  # uncomment to upload to HuggingFace Hub

    train_data = []
    data = reasoning_gym.create_dataset('ab', size=500, seed=42, length=8)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('ab', size=500, seed=42, length=12)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/ab")  # uncomment to upload to HuggingFace Hub

def load_ds():
    train = load_dataset("anonymous/ab", split="validate")
    test = load_dataset("anonymous/ab", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    # init_data()
    parser = argparse.ArgumentParser(description='AB Pattern evaluation with tool generation')
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/ab/test', help='Experiment directory')
