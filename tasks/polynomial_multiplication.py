...
Dataset that generates tasks testing understanding of polynomial multiplication.

    Generates polynomial multiplication problems with:
    - Linear, quadratic, and higher-degree polynomials
    - Various coefficient ranges
    - Expansion and simplification

    The difficulty parameter controls polynomial complexity:
    - Level 1: Simple binomial multiplication like (x + 2)(x + 3)
    - Level 2: More complex polynomials with higher coefficients
    - Level 3+: Higher degree polynomials and multiple terms

    Each task provides:
    - A question asking to multiply and simplify polynomials
    - The correct expanded form
    - Metadata including the original polynomials
...
````
Example 1:
Question: Multiply and simplify: (x + 2)(x + 3). Provide the answer in standard form.
Answer: x^2 + 5x + 6
Metadata: {'source_dataset': 'polynomial_multiplication', 'source_index': 0, 'problem': '(x + 2)(x + 3)', 'difficulty': {'difficulty': 2}}

Example 2:
Question: Multiply and simplify: (2x - 1)(3x + 4). Provide the answer in standard form.
Answer: 6x^2 + 5x - 4
Metadata: {'source_dataset': 'polynomial_multiplication', 'source_index': 1, 'problem': '(2x - 1)(3x + 4)', 'difficulty': {'difficulty': 2}}

Example 3:
Question: Multiply and simplify: (x + 1)(x^2 - x + 1). Provide the answer in standard form.
Answer: x^3 + 1
Metadata: {'source_dataset': 'polynomial_multiplication', 'source_index': 2, 'problem': '(x + 1)(x^2 - x + 1)', 'difficulty': {'difficulty': 2}}
...
def init_data():
    ...
    test_data = []
    data = reasoning_gym.create_dataset('polynomial_multiplication', size=70, seed=42, min_terms=2, max_terms=3, min_value=1, max_value=5, min_degree=1, max_degree=2, min_polynomials=2, max_polynomials=2)
    ...
    data = reasoning_gym.create_dataset('polynomial_multiplication', size=70, seed=42, min_terms=3, max_terms=4, min_value=1, max_value=8, min_degree=2, max_degree=3, min_polynomials=2, max_polynomials=2)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('polynomial_multiplication', size=70, seed=42, min_terms=4, max_terms=5, min_value=1, max_value=10, min_degree=2, max_degree=4, min_polynomials=2, max_polynomials=3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('polynomial_multiplication', size=70, seed=42, min_terms=5, max_terms=6, min_value=1, max_value=12, min_degree=3, max_degree=5, min_polynomials=2, max_polynomials=3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/polynomial_multiplication")  # uncomment to upload to HuggingFace Hub

    val_data = []
    data = reasoning_gym.create_dataset('polynomial_multiplication', size=120, seed=42, min_terms=2, max_terms=3, min_value=1, max_value=5, min_degree=1, max_degree=2, min_polynomials=2, max_polynomials=2)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/polynomial_multiplication")  # uncomment to upload to HuggingFace Hub

    train_data = []
    data = reasoning_gym.create_dataset('polynomial_multiplication', size=500, seed=42, min_terms=2, max_terms=3, min_value=1, max_value=5, min_degree=1, max_degree=2, min_polynomials=2, max_polynomials=2)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('polynomial_multiplication', size=500, seed=42, min_terms=3, max_terms=4, min_value=1, max_value=8, min_degree=2, max_degree=3, min_polynomials=2, max_polynomials=2)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/polynomial_multiplication")  # uncomment to upload to HuggingFace Hub



def load_ds():
    train = load_dataset("anonymous/polynomial_multiplication", split="validate")
    test = load_dataset("anonymous/polynomial_multiplication", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Polynomial Multiplication evaluation with tool generation')
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/polynomial_multiplication/test', help='Experiment directory')
