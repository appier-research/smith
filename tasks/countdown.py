"""
### countdown
Dataset that generates Countdown number game tasks.

    Generates mathematical puzzles where you must use given numbers
    with basic arithmetic operations (+, -, *, /) to reach a target number.
    Each number can be used at most once.

    The difficulty parameter controls expression complexity:
    - Level 2: 4 numbers, target 10-100
    - Level 3: 5 numbers, target 10-150
    - Level 4: 6 numbers, target 10-200

    Each task provides:
    - A question asking to calculate a target using given numbers
    - The correct answer as an arithmetic expression
    - Metadata including the numbers, target, and solution expression
...
````
Example 1:
Question: Calculate 50 using all of these numbers: 12, 87, 70, 95.
Each number may be used at most once.
Answer: 87 - 95 + 70 - 12
Metadata: {'source_dataset': 'countdown', 'source_index': 0, 'numbers': [12, 87, 70, 95], 'target': 50, 'expression': '87 - 95 + 70 - 12', 'difficulty': {'numbers': (4, 4), 'target': (50, 200), 'value': (1, 100)}}

Example 2:
Question: Using all the numbers 75, 8, 38, 28, 70, 95, create an expression that equals 628.
You can only use each number once.
Answer: 8*(38 + 28) + 75 + 95 - 70
Metadata: {'source_dataset': 'countdown', 'source_index': 0, 'numbers': [75, 8, 38, 28, 70, 95], 'target': 628, 'expression': '8*(38 + 28) + 75 + 95 - 70', 'difficulty': {'numbers': (6, 6), 'target': (500, 999), 'value': (1, 100)}}

Example 3:
Question: Find a way to make 205 using all of these numbers: 29, 10, 91, 76, 1.
Each number can only be used once.
Answer: 76 + 29 - 1 + 10 + 91
Metadata: {'source_dataset': 'countdown', 'source_index': 0, 'numbers': [29, 10, 91, 76, 1], 'target': 205, 'expression': '76 + 29 - 1 + 10 + 91', 'difficulty': {'numbers': (5, 6), 'target': (200, 500), 'value': (1, 100)}}
...
def init_data():
    ...
    # Difficulty 2: 4 numbers, easier targets
    print("Generating difficulty 2 test data...")
    data = reasoning_gym.create_dataset('countdown', size=70, seed=42, min_numbers=4, max_numbers=4, min_target=10, max_target=100)
    ...
    # Difficulty 3: 5 numbers, moderate targets
    print("Generating difficulty 3 test data...")
    data = reasoning_gym.create_dataset('countdown', size=70, seed=43, min_numbers=5, max_numbers=5, min_target=10, max_target=150)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    # Difficulty 4: 6 numbers, harder targets
    print("Generating difficulty 4 test data...")
    data = reasoning_gym.create_dataset('countdown', size=70, seed=44, min_numbers=6, max_numbers=6, min_target=10, max_target=200)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/countdown")  # uncomment to upload to HuggingFace Hub

    # Validation data
    print("Generating validation data...")
    val_data = []
    data = reasoning_gym.create_dataset('countdown', size=120, seed=45, min_numbers=4, max_numbers=5, min_target=10, max_target=100)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/countdown")  # uncomment to upload to HuggingFace Hub

    # Training data
    print("Generating training data (difficulty 2)...")
    train_data = []
    data = reasoning_gym.create_dataset('countdown', size=250, seed=46, min_numbers=4, max_numbers=4, min_target=10, max_target=100)
    for i, x in enumerate(data):
        if i % 50 == 0:
    ...
    print("Generating training data (difficulty 3)...")
    data = reasoning_gym.create_dataset('countdown', size=250, seed=47, min_numbers=5, max_numbers=5, min_target=10, max_target=150)
    for i, x in enumerate(data):
        if i % 50 == 0:
            print(f'Progress: {i}/250')
        # use the dataset's `score_answer` method for algorithmic verification
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/countdown")  # uncomment to upload to HuggingFace Hub



def load_ds():
    train = load_dataset("anonymous/countdown", split="validate")
    test = load_dataset("anonymous/countdown", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Countdown evaluation with tool generation')
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/countdown/test', help='Experiment directory')
