...
def init_data():
    ...
    test_data = []
    data = reasoning_gym.create_dataset('lcm', size=70, seed=42, min_numbers=2, max_numbers=2, min_value=1, max_value=50)
    ...
    data = reasoning_gym.create_dataset('lcm', size=70, seed=42, min_numbers=2, max_numbers=3, min_value=50, max_value=200)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('lcm', size=70, seed=42, min_numbers=3, max_numbers=4, min_value=100, max_value=500)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/lcm")  # uncomment to upload to HuggingFace Hub

    val_data = []
    data = reasoning_gym.create_dataset('lcm', size=120, seed=42, min_numbers=2, max_numbers=2, min_value=1, max_value=100)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/lcm")  # uncomment to upload to HuggingFace Hub

    train_data = []
    data = reasoning_gym.create_dataset('lcm', size=500, seed=42, min_numbers=2, max_numbers=2, min_value=1, max_value=50)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('lcm', size=500, seed=42, min_numbers=2, max_numbers=3, min_value=50, max_value=200)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/lcm")  # uncomment to upload to HuggingFace Hub

def load_ds():
    train = load_dataset("anonymous/lcm", split="validate")
    test = load_dataset("anonymous/lcm", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    # init_data()
    parser = argparse.ArgumentParser(description='LCM evaluation with tool generation')
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/lcm/test', help='Experiment directory')
