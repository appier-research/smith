...
def init_data():
    ...
    test_data = []
    data = reasoning_gym.create_dataset('count_bits', size=70, seed=42, min_n=1, max_n=1000000)
    ...
    data = reasoning_gym.create_dataset('count_bits', size=70, seed=42, min_n=1000000, max_n=10000000)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('count_bits', size=70, seed=42, min_n=10000000, max_n=100000000)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/count_bits")  # uncomment to upload to HuggingFace Hub

    val_data = []
    data = reasoning_gym.create_dataset('count_bits', size=120, seed=42, min_n=1, max_n=5000000)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/count_bits")  # uncomment to upload to HuggingFace Hub

    train_data = []
    data = reasoning_gym.create_dataset('count_bits', size=500, seed=42, min_n=1, max_n=1000000)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('count_bits', size=500, seed=42, min_n=1000000, max_n=10000000)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/count_bits")  # uncomment to upload to HuggingFace Hub

def load_ds():
    train = load_dataset("anonymous/count_bits", split="validate")
    test = load_dataset("anonymous/count_bits", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    # init_data()
    parser = argparse.ArgumentParser(description='Count Bits evaluation with tool generation')
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/count_bits/test', help='Experiment directory')
