...
def init_data():
    ...
    test_data = []
    data = reasoning_gym.create_dataset('isomorphic_strings', size=70, seed=42, min_string_length=10, max_string_length=19)
    ...
    data = reasoning_gym.create_dataset('isomorphic_strings', size=140, seed=42, min_string_length=20, max_string_length=30)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0    
    ...
    data = reasoning_gym.create_dataset('isomorphic_strings', size=140, seed=42, min_string_length=31, max_string_length=40)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0    
    ...
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/isomorphic_strings")  # uncomment to upload to HuggingFace Hub

    val_data = []
    data = reasoning_gym.create_dataset('isomorphic_strings', size=120, seed=42, min_string_length=3, max_string_length=12)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/isomorphic_strings")  # uncomment to upload to HuggingFace Hub

    train_data = []
    data = reasoning_gym.create_dataset('isomorphic_strings', size=500, seed=42, min_string_length=3, max_string_length=12)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('isomorphic_strings', size=500, seed=42, min_string_length=10, max_string_length=19)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/isomorphic_strings")  # uncomment to upload to HuggingFace Hub

def load_ds():
    train = load_dataset("anonymous/isomorphic_strings", split="validate")
    test = load_dataset("anonymous/isomorphic_strings", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    # init_data()
    parser = argparse.ArgumentParser(description='Bitwise Arithmetic evaluation with tool generation')
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/isomorphic_string/test', help='Experiment directory')
