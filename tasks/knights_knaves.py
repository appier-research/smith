""" ### knights_knaves
Generates Knights and Knaves logic puzzle tasks

Knights always tell the truth, knaves always lie. Determine who is a knight and who is a knave.
...
````
Example 1:
Question: You meet two people: Alice and Bob. Alice says "Bob is a knave." Bob says "Alice is a knight." Who is a knight and who is a knave?
Answer: Alice is a knight, Bob is a knight
Metadata: {'source_dataset': 'knights_knaves', 'source_index': 0, 'num_characters': 2, 'difficulty': {'num_characters': 2}}

Example 2:
Question: You meet two people: Alice and Bob. Alice says "I am a knave." What can you determine about Alice?
Answer: Alice is a knave
Metadata: {'source_dataset': 'knights_knaves', 'source_index': 1, 'num_characters': 1, 'difficulty': {'num_characters': 1}}

Example 3:
Question: You meet three people: Alice, Bob, and Charlie. Alice says "Bob is a knave." Bob says "Charlie is a knight." Charlie says "Alice and Bob are both knaves." Who is a knight and who is a knave?
Answer: Alice is a knight, Bob is a knave, Charlie is a knave
Metadata: {'source_dataset': 'knights_knaves', 'source_index': 2, 'num_characters': 3, 'difficulty': {'num_characters': 3}}
````
"""
import json
import os
...
def init_data():
    ...
    test_data = []
    data = reasoning_gym.create_dataset('knights_knaves', size=70, seed=42, n_people=2, depth_constraint=2, width_constraint=3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
        # Convert tuple statements to string for serialization
        if 'statements' in x['metadata'] and isinstance(x['metadata']['statements'], tuple):
            x['metadata']['statements'] = str(x['metadata']['statements'])
        x['difficulty'] = 2
        test_data.append(x)

    data = reasoning_gym.create_dataset('knights_knaves', size=70, seed=42, n_people=3, depth_constraint=3, width_constraint=4)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('knights_knaves', size=70, seed=42, n_people=4, depth_constraint=4, width_constraint=5)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/knights_knaves")  # uncomment to upload to HuggingFace Hub

    val_data = []
    data = reasoning_gym.create_dataset('knights_knaves', size=120, seed=42, n_people=2, depth_constraint=2, width_constraint=3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/knights_knaves")  # uncomment to upload to HuggingFace Hub

    train_data = []
    data = reasoning_gym.create_dataset('knights_knaves', size=500, seed=42, n_people=2, depth_constraint=2, width_constraint=3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('knights_knaves', size=500, seed=42, n_people=3, depth_constraint=3, width_constraint=4)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/knights_knaves")  # uncomment to upload to HuggingFace Hub

def load_ds():
    train = load_dataset("anonymous/knights_knaves", split="validate")
    test = load_dataset("anonymous/knights_knaves", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    # init_data()
    parser = argparse.ArgumentParser(description='Knights and Knaves evaluation with tool generation')
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/knights_knaves/test', help='Experiment directory')
