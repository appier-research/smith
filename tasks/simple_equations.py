"""
### simple_equations
Dataset that generates tasks testing understanding of simple linear equations.

    Generates simple linear equations with:
    - One or two operations
    - Integer and decimal coefficients
    - Simple algebraic manipulation

    The difficulty parameter controls equation complexity:
    - Level 1: Basic equations like 2x + 3 = 7
    - Level 2: Equations with fractions or decimals
    - Level 3+: Multi-step equations with distribution

    Each task provides:
    - A question asking to solve for the variable
    - The correct solution
    - Metadata including the equation

    The dataset verifies answers by substitution.
...
Answer: 2
Metadata: {'source_dataset': 'simple_equations', 'source_index': 0, 'equation': '2x + 3 = 7', 'difficulty': {'difficulty': 2}}

Example 2:
Question: Solve for x: 5x - 10 = 15
Answer: 5
Metadata: {'source_dataset': 'simple_equations', 'source_index': 1, 'equation': '5x - 10 = 15', 'difficulty': {'difficulty': 2}}

Example 3:
Question: Solve for x: 3(x + 2) = 18
Answer: 4
Metadata: {'source_dataset': 'simple_equations', 'source_index': 2, 'equation': '3(x + 2) = 18', 'difficulty': {'difficulty': 2}}
...
def init_data():
    ...
    test_data = []
    data = reasoning_gym.create_dataset('simple_equations', size=70, seed=42, min_terms=2, max_terms=3, min_value=1, max_value=10)
    ...
    data = reasoning_gym.create_dataset('simple_equations', size=70, seed=42, min_terms=3, max_terms=4, min_value=1, max_value=20)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('simple_equations', size=70, seed=42, min_terms=4, max_terms=5, min_value=1, max_value=30)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('simple_equations', size=70, seed=42, min_terms=5, max_terms=6, min_value=1, max_value=50)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/simple_equations")  # uncomment to upload to HuggingFace Hub

    val_data = []
    data = reasoning_gym.create_dataset('simple_equations', size=120, seed=42, min_terms=2, max_terms=3, min_value=1, max_value=10)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/simple_equations")  # uncomment to upload to HuggingFace Hub

    train_data = []
    data = reasoning_gym.create_dataset('simple_equations', size=500, seed=42, min_terms=2, max_terms=3, min_value=1, max_value=10)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('simple_equations', size=500, seed=42, min_terms=3, max_terms=4, min_value=1, max_value=20)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/simple_equations")  # uncomment to upload to HuggingFace Hub



def load_ds():
    train = load_dataset("anonymous/simple_equations", split="validate")
    test = load_dataset("anonymous/simple_equations", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Simple Equations evaluation with tool generation')
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/simple_equations/test', help='Experiment directory')
