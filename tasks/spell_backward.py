...
def init_data():
    ...
    test_data = []
    data = reasoning_gym.create_dataset('spell_backward', size=70, seed=42, min_word_len=3, max_word_len=6)
    ...
    data = reasoning_gym.create_dataset('spell_backward', size=70, seed=42, min_word_len=6, max_word_len=9)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('spell_backward', size=70, seed=42, min_word_len=9, max_word_len=12)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/spell_backward")  # uncomment to upload to HuggingFace Hub

    val_data = []
    data = reasoning_gym.create_dataset('spell_backward', size=120, seed=42, min_word_len=3, max_word_len=10)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/spell_backward")  # uncomment to upload to HuggingFace Hub

    train_data = []
    data = reasoning_gym.create_dataset('spell_backward', size=500, seed=42, min_word_len=3, max_word_len=6)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('spell_backward', size=500, seed=42, min_word_len=6, max_word_len=9)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/spell_backward")  # uncomment to upload to HuggingFace Hub

def load_ds():
    train = load_dataset("anonymous/spell_backward", split="validate")
    test = load_dataset("anonymous/spell_backward", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    # init_data()
    parser = argparse.ArgumentParser(description='Spell Backward evaluation with tool generation')
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/spell_backward/test', help='Experiment directory')
