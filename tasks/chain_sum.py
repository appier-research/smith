...
def init_data():
    ...
    test_data = []
    data = reasoning_gym.create_dataset('chain_sum', size=70, seed=42, min_terms=2, max_terms=4, min_digits=1, max_digits=2)
    ...
    data = reasoning_gym.create_dataset('chain_sum', size=70, seed=42, min_terms=3, max_terms=5, min_digits=2, max_digits=4)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('chain_sum', size=70, seed=42, min_terms=4, max_terms=6, min_digits=3, max_digits=5)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(test_data, split="test")  # .push_to_hub("anonymous/chain_sum")  # uncomment to upload to HuggingFace Hub

    val_data = []
    data = reasoning_gym.create_dataset('chain_sum', size=120, seed=42, min_terms=2, max_terms=4, min_digits=1, max_digits=3)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(val_data, split="validate")  # .push_to_hub("anonymous/chain_sum")  # uncomment to upload to HuggingFace Hub

    train_data = []
    data = reasoning_gym.create_dataset('chain_sum', size=500, seed=42, min_terms=2, max_terms=4, min_digits=1, max_digits=2)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    data = reasoning_gym.create_dataset('chain_sum', size=500, seed=42, min_terms=3, max_terms=5, min_digits=2, max_digits=4)
    for i, x in enumerate(data):
        print('metadata:', x['metadata'])
        # use the dataset's `score_answer` method for algorithmic verification
        assert data.score_answer(answer=x['answer'], entry=x) == 1.0
    ...
    Dataset.from_list(train_data, split="train")  # .push_to_hub("anonymous/chain_sum")  # uncomment to upload to HuggingFace Hub

def load_ds():
    train = load_dataset("anonymous/chain_sum", split="validate")
    test = load_dataset("anonymous/chain_sum", split="test")
    test_a = []
    for t in test:
        t['inputs'] = t['question']
        t['targets'] = t['answer']
...
if __name__ == "__main__":
    # init_data()
    parser = argparse.ArgumentParser(description='Chain Sum evaluation with tool generation')
    ...
    parser.add_argument('--experiment_dir', type=str, default='experiments/chain_sum/test', help='Experiment directory')
